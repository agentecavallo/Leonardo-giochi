...
ANIMALI = {
    ...
    "Rana 🐸": "Anfibio", "Rospo 🐸": "Anfibio", "Salamandra 🦎💧": "Anfibio", "Axolotl (Assolotto) 🦎🎀": "Anfibio",
...
# --- DATABASE ITALIA (Capoluoghi di Provincia) ---
MAPPA_ITALIA = {
...
def controlla_tempo(prefisso, giusta, testo_domanda):
    ...
    if errori:
        ...
        for err in errori:
            ...
            <div class="ripasso-box">
                <b>{err['domanda']}</b><br>
                Hai risposto: <span style='color: #ff4b4b;'>{err['risposta']}</span> ❌<br>
                Risposta giusta: <span style='color: #00ff00;'>{err['giusta']}</span> ✅
            </div>
            """, unsafe_allow_html=True)
...
def verifica_tab(scelta):
    ...
    if controlla_tempo('tab', giusta, testo):
...
def verifica_ani(scelta):
    ...
    if controlla_tempo('ani', giusta, st.session_state.animale_corr):
        if scelta == giusta:
...
def verifica_ban(scelta):
    ...
    if controlla_tempo('ban', giusta, f"Bandiera (era {giusta})"):
        if scelta == giusta:
...
def verifica_ana(scelta):
    ...
    if controlla_tempo('ana', giusta, testo):
        if scelta == giusta:
...
def verifica_ita(scelta):
    ...
    if controlla_tempo('ita', giusta, testo):
        if scelta == giusta:
            st.session_state.msg_ita = f"🚀 BRAVISSIMO! {testo} è in {giusta}"
